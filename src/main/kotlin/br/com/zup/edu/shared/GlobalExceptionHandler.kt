...
/**
 * ExceptionHandler interface implementation using Interceptor
 */
...
class GlobalExceptionHandler : ExceptionHandler<StatusRuntimeException, HttpResponse<Any>> {
    ...
    override fun handle(request: HttpRequest<*>, exception: StatusRuntimeException): HttpResponse<Any> {
        ...
        val (httpStatus, message) = when (statusCode) {
            ...
            Status.INVALID_ARGUMENT.code -> Pair(HttpStatus.BAD_REQUEST, "invalid input data")
            Status.ALREADY_EXISTS.code -> Pair(HttpStatus.UNPROCESSABLE_ENTITY, statusDescription)
            else -> {
                LOGGER.info("unexpected error '${exception.javaClass.name}' to process the request", exception)
                Pair(HttpStatus.INTERNAL_SERVER_ERROR, "the request could not be completed due to the error: $statusDescription (${statusCode})")
            }
        }
